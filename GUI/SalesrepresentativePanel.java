...
public class SalesrepresentativePanel implements ActionListener {
    ...
    String whatCustS = "SELECT FIRST, MINIT, LAST FROM rkraft3db.DBP_CUSTOMER WHERE C_SSN IN (SELECT CUST_SSN FROM rkraft3db.DBP_SALE WHERE rkraft3db.SALE_VIN IN (SELECT VIN FROM rkraft3db.DBP_CAR WHERE MAKE = '#'))";
    String whoBoughtS = "SELECT FIRST, LAST FROM rkraft3db.DBP_CUSTOMER WHERE C_SSN IN (SELECT CUST_SSN FROM rkraft3db.DBP_SALE WHERE rkraft3db.SREP_SSN IN (SELECT SR_SSN FROM rkraft3db.DBP_SALES_REP WHERE SR_SSN IN (SELECT SSN FROM rkraft3db.DBP_EMPLOYEE WHERE FIRST = '!' AND LAST = '^')))";
    String where = " WHERE ";
    int sqlType;
    int deleteCount = 0;
